...
def salvar_ranking_movimentacao():
    ...
    with engine.begin() as connection:
        ...
        # Query otimizada para obter o ranking de movimentação
        query = text("""
            WITH movimentacao_agregada AS (
                SELECT 
                    ...
                    MAX(i.vl_total) as vl_total  -- Pega o valor mais recente
                FROM INFORMACAO_DIARIA i
                ...
                WHERE i.dt_comptc BETWEEN :data_inicio AND :data_fim
                  ...
                  AND i.vl_total > 0  -- Filtra apenas fundos com patrimônio
            ...
            SELECT 
                cnpj_fundo_classe,
                tp_fundo_classe,
                :ultima_data as dt_comptc,  -- Usa a data mais recente
                denominacao_social,
                vl_total,
                total_resgates,
                total_aportes,
                fluxo_liquido,
                CASE 
                    WHEN vl_total > 0 THEN 
                        (fluxo_liquido / vl_total) * 100
                    ELSE 0 
                END as percentual_fluxo_liquido,
                ROW_NUMBER() OVER (ORDER BY fluxo_liquido DESC) as ranking
